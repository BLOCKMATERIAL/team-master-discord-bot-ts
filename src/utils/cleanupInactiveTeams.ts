    ...
    for (const team of teams) {
      ...
      if (timeSinceCreation > INACTIVE_THRESHOLD) {
        try {
          ...
          logger.warn(
            `Failed to delete message for team ${team.teamId}: ${error}`,
          );
        }
        ...
        if (team.voiceChannelId) {
          try {
            ...
            if (voiceChannel) {
              ...
              logger.info(
                `Deleted voice channel for inactive team ${team.teamId}`,
              );
            }
          } catch (error) {
            logger.warn(
              `Failed to delete voice channel for team ${team.teamId}: ${error}`,
            );
          }
        }
