...
export async function handleInitCommand(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        if (games.length === 0) {
            ...
            await interaction.reply({ content: 'На жаль, зараз немає доступних ігор для вибору. Спробуйте пізніше.', ephemeral: true });
            return;
        }
        ...
        const content = userGames.length > 0
...
export async function handleInitGameSelection(interaction: StringSelectMenuInteraction) {
    ...
    try {
        ...
        const gameNames = await Game.find({ value: { $in: selectedGames } }).then(games => games.map(g => g.name));
        ...
        logger.info(`User ${userId} (${username}, ${displayName}) updated their game preferences: ${gameNames.join(', ')}`);
    } catch (error) {
        logger.error('Error in handleInitGameSelection:', error);
        await interaction.update({ content: 'Виникла помилка при збереженні ваших ігор. Спробуйте ще раз пізніше.', components: [] });
    }
}