...
export async function handleInviteCommand(interaction: ChatInputCommandInteraction) {
    try {
        ...
        if (team.players.length < team.slots) {
            ...
            logger.info(`User ${interaction.user.id} invited ${playerToInvite.id} ${playerToInvite.username} to team ${teamId}`);
            await interaction.reply({ content: `Гравця ${playerToInvite} запрошено до команди.`, ephemeral: true });
        } else {
            team.reserve.push(newPlayer);
            logger.info(`User ${interaction.user.id} invited ${playerToInvite.id} ${playerToInvite.username} to reserve of team ${teamId}`);
        ...
        try {
            ...
            logger.warn(`Failed to send invitation message to user ${playerToInvite.id}: ${error}`);
            await interaction.followUp({ content: `Не вдалося надіслати повідомлення гравцю ${playerToInvite}. Можливо, у них вимкнені особисті повідомлення.`, ephemeral: true });
        }
