...
export async function handleListTeamsCommand(
  ...
  try {
    ...
    if (teams.length === 0) {
      await interaction.reply({
        content: 'На даний момент немає активних команд.',
        ephemeral: true,
      });
      return;
    }
    ...
    await interaction.reply({
      content:
        'Виникла помилка при отриманні списку команд. Спробуйте ще раз пізніше.',
      ephemeral: true,
    });
  }
}
