...
export async function handleCreateCommand(interaction: ChatInputCommandInteraction) {
    try {
        if (await isUserInAnyTeam(interaction.user.id)) {
            ...
            logger.info(`User ${interaction.user.id} ${interaction.user.username} tried to create a team but is already in a team`);
            return;
        }
        ...
        if (games.length === 0) {
            ...
            logger.warn('No games found in the database when attempting to create a team');
            return;
        }
        ...
        await interaction.reply({ content: 'Виникла помилка при створенні команди. Спробуйте ще раз пізніше.', ephemeral: true });
    }
}