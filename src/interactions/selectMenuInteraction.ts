    ...
    try {
      ...
      if (!game) {
        await interaction.update({
          content: 'Обрана гра не знайдена. Спробуйте ще раз.',
          components: [],
        });
        return;
      }
      ...
      await interaction.update({
        content: 'Виникла помилка при виборі гри. Спробуйте ще раз пізніше.',
        components: [],
      });
    }
  }
...
export async function showCreateTeamModal(
  interaction: StringSelectMenuInteraction,
  game: string,
) {
  ...
  const slotsRow = new ActionRowBuilder<TextInputBuilder>().addComponents(
    slotsInput,
  );
  const startTimeRow = new ActionRowBuilder<TextInputBuilder>().addComponents(
    startTimeInput,
  );
  ...
  try {
    ...
    await interaction.update({
      content:
        'Виникла помилка при відкритті форми створення команди. Спробуйте ще раз пізніше.',
      components: [],
    });
  }
}
