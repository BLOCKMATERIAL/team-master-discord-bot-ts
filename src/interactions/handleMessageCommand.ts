...
export async function handleMessageCommand(interaction: ChatInputCommandInteraction) {
    ...
    if (!teamId || !message) {
        await interaction.reply({ content: 'Будь ласка, вкажіть ID команди та повідомлення.', ephemeral: true });
        return;
    }
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: 'Час відправлення', value: new Date().toLocaleString('uk-UA', { timeZone: 'Europe/Kiev' }) }
        )
    ...
    try {
        ...
        logger.error(`Failed to send message to team channel ${team.channelId}: ${error}`);
    }

    await interaction.reply({ content: `Повідомлення успішно надіслано команді ${teamId}.`, ephemeral: true });
